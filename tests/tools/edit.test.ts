...
describe("handleEdit", () => {
  ...
  test("preserves CRLF line endings after edit", async () => {
    ...
    const result = await handleEdit({
      file_path: crlfFile,
      checksum: cs,
      edits: [{ range: `2:${h2}..2:${h2}`, content: ["replaced"] }],
      projectDir: testDir,
    });
    expect(result.isError).toBeUndefined();
  ...
  test("rejects directory path", async () => {
    const result = await handleEdit({
      ...
      edits: [{ range: "1:aa..1:aa", content: ["x"] }],
      projectDir: testDir,
    });
    expect(result.isError).toBe(true);
  ...
  test("rejects overlapping ranges", async () => {
    ...
    const result = await handleEdit({
      ...
      checksum: cs,
      edits: [
        { range: `1:${h1}..2:${h2}`, content: ["A"] },
        { range: `2:${h2}..2:${h2}`, content: ["B"] },
      ],
      projectDir: testDir,
    });
    expect(result.isError).toBe(true);
  ...
  test("preserves absence of trailing newline", async () => {
    ...
    const result = await handleEdit({
      ...
      checksum: cs,
      edits: [{ range: `1:${h1}..1:${h1}`, content: ["replaced"] }],
      projectDir: testDir,
    });
    expect(result.isError).toBeUndefined();
  ...
  test("denies editing .env file", async () => {
    ...
    const result = await handleEdit({
      file_path: envFile,
      checksum: cs,
      edits: [{ range: `1:${h}..1:${h}`, content: ["hacked"] }],
      projectDir: testDir,
    });
